...
from modeling import modeling_greaselm, modeling_greaselm_head
...
# probably better to turn this into command line argument
USE_BIGGER_HEAD = True
...
def construct_model(args, kg):
    ...
    cp_emb = np.concatenate(cp_emb, 1) # problem line
    ...
    if USE_BIGGER_HEAD:
        model_class = modeling_greaselm_head
    else:
        model_class = modeling_greaselm

    

    model = model_class.GreaseLM(args, args.encoder, k=args.k, n_ntype=n_ntype, n_etype=n_etype, n_concept=concept_num,
    ...
    print("not quite here")

    return model


def sep_params(model, loaded_roberta_keys): # TO CHANGE: which of llama parameters should be frozen vs not frozen?
...
def train(args, resume, has_test_split, devices, kg):
    ...
    model = construct_model(args, kg) # problem line
...
def evaluate(args, has_test_split, devices, kg):
    ...
    model.to(devices[1])
    print("set model to device and eval")

    model.lmgnn.concept_emb.to(devices[0])
    model.eval()

    print("what's the problem with this terminal??") # literally just had to re-run and it worked
    ...
    print("should be in the clear by here")
