...
import sys
sys.path.append("/home/mwg37/GreaseLM/")
...
from transformers.utils.hub import cached_file as cached_path # for transformers version 4.31.0
...
ModelClass = modeling_llama.LlamaModel
    
print ('ModelClass', ModelClass)

class GreaseLM(nn.Module):

    def __init__(self, args={}, model_name="meta-llama/Llama-2-7b-hf", k=5, n_ntype=4, n_etype=38,
...
class TextKGMessagePassing(ModelClass):
    ...
    def __init__(self, config, args={}, k=5, n_ntype=4, n_etype=38, dropout=0.2, concept_dim=200, ie_dim=200, p_fc=0.2, info_exchange=True, ie_layer_num=1, sep_ie_layers=False):
        ...
        self.encoder = LLaMAGAT(config, k=k, n_ntype=n_ntype, n_etype=n_etype, hidden_size=concept_dim, dropout=dropout, concept_dim=concept_dim, ie_dim=ie_dim, p_fc=p_fc, info_exchange=info_exchange, ie_layer_num=ie_layer_num, sep_ie_layers=sep_ie_layers)
...
def test_TextKGMessagePassing(device):
    model = TextKGMessagePassing.from_pretrained("meta-llama/Llama-2-7b-hf", output_hidden_states=True).to(device)
    inputs = model.get_fake_inputs(device)
    outputs = model(*inputs)
    model.check_outputs(*outputs)


class LLaMAGAT(modeling_llama.LlamaModel): 
    ...
    def forward(self, hidden_states, attention_mask, special_tokens_mask, head_mask, _X, edge_index, edge_type, _node_type, _node_feature_extra, special_nodes_mask, output_attentions=False, output_hidden_states=True):
        ...
        all_hidden_states = ()
        all_attentions = ()
        for i, layer_module in enumerate(self.layers): # self.layers instead of self.layer
            # LM
...
def test_LLaMAGAT(device):
    config, _ = modeling_llama.LlamaModel.config_class.from_pretrained(
        "meta-llama/Llama-2-7b-hf",
        cache_dir=None, return_unused_kwargs=True,
        force_download=False,
        output_hidden_states=True
    )
    model = LLaMAGAT(config, sep_ie_layers=True).to(device)
    inputs = model.get_fake_inputs(device)
    outputs = model(*inputs)
    model.check_outputs(*outputs)
